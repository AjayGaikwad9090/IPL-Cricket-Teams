...
let confirmTeam = newlink.searchParams.get("name");
...
var playedCrads = document.getElementById("teamGrid")
var tableData = document.getElementById("team-table")

for (var i = 0; i < playerDetails.length; i++) {
    if (confirmTeam == playerDetails[i].from) {
        var isPlay = ""
        ...
        var currPlayer = playerDetails[i].playerName
        playerCount++
        playedCrads.innerHTML += `
        <div onclick="playerCardClick('${currPlayer}')" class="player-card">
        <img src="${playerDetails[i].playerImg}" alt="">
        <div class="text-info">
        <p>Player Name: ${playerDetails[i].playerName}</p>
        <p>Team Name: ${playerDetails[i].playerTeam}</p>
        <p>Price: ${playerDetails[i].price}</p>
        <p>Role: ${playerDetails[i].description}</p>
        <p>Playing: ${playerDetails[i].isPlaying}</p>
        </div>
        </div>
        `
    }
...
function playerCardClick(result) {
    window.open(`./playerinfo.html?name=${result}`, "_self")
}

// top Bowler
var topBowler = ''
for (let i = 0; i < playerDetails.length; i++) {
    if (playerDetails[i].description == 'Bowler' && playerDetails[i].from == confirmTeam) {
        topBowler = playerDetails[i].playerName
        break
    } else {
        topBowler = "no Player"
    }
...
var topBatsman = ''
for (let i = 0; i < playerDetails.length; i++) {
    if (playerDetails[i].description == 'Batsman' && playerDetails[i].from == confirmTeam) {
        topBatsman = playerDetails[i].playerName
        break
    } else {
