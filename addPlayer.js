...
// salecting team option in add player page from
var choosTeam = document.getElementById("input6")
for (var i = 0; i < teamDetails.length; i++) {
    choosTeam.innerHTML += `
    <option value="${teamDetails[i].teamFullName}">${teamDetails[i].teamFullName}</option>
    `
}
// salecting team option in add player page from
...
$("#addPlayerform").submit(function (e) {
    e.preventDefault();
    let user_input = $("#input6").val();
    let cutName = '';
    for (let i = 0; i < user_input.length; i++) {
        if (i == 0) {
            cutName += user_input[i++].toUpperCase();
            continue;
