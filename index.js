...
// setting data tu the local storage start
let teamDetails = [];
let playerDetails = [];
...
// setting data tu the local storage end

// Rendering data tu the html start
var teamCard = document.getElementById("teamsContainer")
for (var i = 0; i < teamDetails.length; i++) {
    teamCard.innerHTML += `
    <div onclick="teamCardClick('${i}')" class="team-card">
    <img src="${teamDetails[i].teamIcon}" alt="" class="teamLogo" />
    <div class="teanInfo-text">
    <p>${teamDetails[i].teamFullName}</p>
    <p><i class="fa-solid fa-trophy"></i>Won Count: ${teamDetails[i].WonCount}</p>
    </div>
    </div>
    `
}
// card click event strt
function teamCardClick(result) {
    var cardClick = teamDetails[result].sName
    window.open(`./teamPage.html?name=${cardClick}`, "_self")
}

// card click event end

// Rendering data tu the html end
...
//     newArray.push(teamDetails[i].sName)
// }
...
//         })
//         searchBox.classList.add("Current");
//         displaySugg(emptyArray);
//         let totalList = autoCom.querySelectorAll("li");
//         for (let i = 0; i < totalList.length; i++) {
//             totalList[i].setAttribute("onclick", "currLi(this)")
//         }
//     } else {
//         searchBox.classList.remove("Current")
//     }
// }

// function currLi(element) {
//     let selectText = element.textContent;
//     searchInput.value = selectText;
//     icon.onclick = () => {
//         window.open(`./teamPage.html?name=${element.textContent}`, '_self')
//     }
//     searchBox.classList.remove("Current")
// }
...
//     let listContent;
//     if (!list.length) {
//         inputByUser = inputBox.value;
//         listContent = `<li>${inputByUser}</li>`
//     } else {
//         listContent = list.join('');
//     }
//     autoCom.innerHTML = listContent;
// }
// Search  Bar end