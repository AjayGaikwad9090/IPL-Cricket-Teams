...
// add team form submit code goes here
$("#addTeamFrom").submit(function (e) {
    e.preventDefault();
    let user_input = $("#input1").val();
    let smName = '';
    for (let i = 0; i < user_input.length; i++) {
        if (i == 0) {
            smName += user_input[i++].toUpperCase();
            continue;
    ...
    teamDetails.push(addteamData);
    localStorage.setItem("teamArray", JSON.stringify(teamDetails));

    location.href = `./teamPage.html?name=${addteamData.sName}`;
})
// add team form submit code goes here
